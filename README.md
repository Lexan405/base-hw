...
## 2.1. Пополнение баланса счёта телефона (ручное)

Для успешного завершения транзакции пополнения баланса необходимо выполнить следующие **шесть действий** в рамках одной транзакции (с соблюдением свойств ACID — атомарность, согласованность, изолированность, долговечность):

1. **Проверка наличия средств на счёте пользователя**  
   Система проверяет, достаточно ли денег на банковской карте или электронном кошельке пользователя для совершения платежа.

2. **Блокировка (резервирование) средств**  
   Необходимая сумма временно блокируется на счёте пользователя, чтобы избежать двойного списания.

3. **Списание средств с платёжного средства**  
   Система списывает деньги с карты/кошелька пользователя и переводит их на счёт оператора связи (или платёжного агрегатора).

4. **Обновление баланса телефона**  
   Баланс абонента в базе данных оператора увеличивается на сумму пополнения.

5. **Фиксация транзакции (COMMIT)**  
   Все изменения (списание и зачисление) подтверждаются и становятся постоянными в базе данных.

6. **Генерация подтверждающего уведомления**  
   Пользователю отправляется SMS или push-уведомление о успешном пополнении, а также создаётся запись в истории операций.

> Если на любом этапе возникает ошибка (например, недостаточно средств или сбой связи), транзакция **откатывается (ROLLBACK)**, и все изменения отменяются — баланс и счёт пользователя остаются без изменений.

---

## 2.1.* Пополнение через автоплатёж

Если пополнение происходит через **автоплатёж**, последовательность действий похожа, но с дополнительными проверками и автоматизацией:

1. **Проверка условия срабатывания автоплатежа**  
   Система проверяет, достиг ли баланс порогового значения (например, ≤ 50 ₽) или наступил запланированный день оплаты.

2. **Автоматическая проверка наличия средств и валидности платёжного метода**  
   Проверяется, активна ли привязанная карта, не истёк ли её срок, и достаточно ли средств.

3. **Авторизация платежа через платёжную систему**  
   Отправляется запрос в банк или платёжный шлюз для автоматического списания средств без участия пользователя.

4. **Списание средств и зачисление на лицевой счёт абонента**  
   Деньги переводятся, и баланс телефона обновляется в базе оператора.

5. **Фиксация транзакции и логирование**  
   Все операции подтверждаются, создаются записи в журналах автоплатежей и финансовой системы.

6. **Отправка уведомления пользователю**  
   Абонент получает автоматическое уведомление: «Автоплатёж выполнен: +300 ₽. Текущий баланс — 350 ₽».

> Особенность автоплатежа: если транзакция не удалась (например, нет денег), система может **повторить попытку позже** или **отправить предупреждение** о необходимости вручную пополнить счёт.
